 ...
 */
class m160704_000000_retour_addReferrerStats extends BaseMigration
{
    /**
    ...
    public function safeUp()
    {
        ...
        $newColumns = array (
            'referrerUrl' => ColumnType::Varchar
        );

        $this->_addColumnsAfter("retour_stats", $newColumns, "redirectSrcUrl");
    ...
    private function _addColumnsAfter($tableName, $newColumns, $afterColumnHandle)
    {
        ...
        foreach ($newColumns as $columnName => $columnType)
        {
            ...
            if (!craft()->db->columnExists($tableName, $columnName))
            {
                ...
                RetourPlugin::log("Created the `$columnName` in the `$tableName` table.", LogLevel::Info, true);
            ...
            // if the column already exists in the table
                ...
                RetourPlugin::log("Column `$columnName` already exists in the `$tableName` table.", LogLevel::Info, true);
